...
import hyperdrive_py
import pytest
from hyperdrive_py.pypechain_types.IHyperdriveTypes import Fees, PoolConfig, PoolInfo
...
def test_get_max_spot_price():
    ...
    max_spot_price = hyperdrive_py.get_max_spot_price(POOL_CONFIG, POOL_INFO)
...
def test_get_spot_price_after_long():
    ...
    spot_price = hyperdrive_py.get_spot_price_after_long(POOL_CONFIG, POOL_INFO, long_amount=str(1_000 * 10**18))
...
def test_get_solvency():
    """test get_max_spot_rate."""
    solvency = hyperdrive_py.get_solvency(POOL_CONFIG, POOL_INFO)
...
def test_get_spot_rate():
    """test get_spot_rate."""
    spot_rate = hyperdrive_py.get_spot_rate(POOL_CONFIG, POOL_INFO)
...
def test_to_checkpoint():
    """test to_checkpoint."""
    checkpoint_time = hyperdrive_py.to_checkpoint(POOL_CONFIG, POOL_INFO, time=str(100))
...
def test_get_spot_price():
    """test get_spot_price."""
    spot_price = hyperdrive_py.get_spot_price(POOL_CONFIG, POOL_INFO)
...
def test_get_time_stretch():
    ...
    time_stretch = hyperdrive_py.get_time_stretch(
        hyperdrive_py.get_spot_rate(POOL_CONFIG, POOL_INFO),
    )
...
def test_get_effective_share_reserves():
    ...
    effective_share_reserves = hyperdrive_py.get_effective_share_reserves(
...
def test_calculate_bonds_given_shares_and_rate():
    ...
    bonds = hyperdrive_py.calculate_bonds_given_shares_and_rate(
...
def test_calculate_bonds_out_given_shares_in_down():
    ...
    out = hyperdrive_py.calculate_bonds_out_given_shares_in_down(POOL_CONFIG, POOL_INFO, amount_in)
    assert int(out) > 0
...
def test_calculate_shares_in_given_bonds_out_up():
    ...
    amount_in = str(1_000 * 10**18)
    out = hyperdrive_py.calculate_shares_in_given_bonds_out_up(POOL_CONFIG, POOL_INFO, amount_in)
    assert int(out) > 0
...
def test_calculate_shares_in_given_bonds_out_down():
    ...
    amount_in = str(1_000 * 10**18)
    out = hyperdrive_py.calculate_shares_in_given_bonds_out_down(POOL_CONFIG, POOL_INFO, amount_in)
    assert int(out) > 0
...
def test_calculate_shares_out_given_bonds_in_down():
    ...
    amount_in = str(1_000 * 10**18)
    out = hyperdrive_py.calculate_shares_out_given_bonds_in_down(POOL_CONFIG, POOL_INFO, amount_in)
    assert int(out) > 0


def test_calculate_max_buy():
    """Test calculate_max_buy."""
    out = hyperdrive_py.calculate_max_buy(POOL_CONFIG, POOL_INFO)
    assert int(out) > 0
...
def test_calculate_max_sell():
    ...
    out = hyperdrive_py.calculate_max_sell(POOL_CONFIG, POOL_INFO, minimum_share_reserves)
    assert int(out) > 0
...
def test_get_long_amount():
    ...
    long_amount = hyperdrive_py.get_long_amount(POOL_CONFIG, POOL_INFO, base_amount)
...
def test_get_short_deposit():
    ...
    base_required = hyperdrive_py.get_short_deposit(POOL_CONFIG, POOL_INFO, short_amount, spot_price, open_share_price)
    assert int(base_required) > 0
    base_required_default_share_price = hyperdrive_py.get_short_deposit(
        POOL_CONFIG, POOL_INFO, short_amount, spot_price, None
    )
    assert int(base_required_default_share_price) > 0
    assert base_required_default_share_price == hyperdrive_py.get_short_deposit(
...
def test_max_long():
    ...
    max_iterations = 20
    max_long = hyperdrive_py.get_max_long(POOL_CONFIG, POOL_INFO, budget, checkpoint_exposure, max_iterations)
...
def test_max_long_fail_conversion():
    ...
    with pytest.raises(ValueError, match="Failed to convert budget string to U256"):
        hyperdrive_py.get_max_long(POOL_CONFIG, POOL_INFO, budget, checkpoint_exposure, max_iterations)
    budget = "1.23"
    checkpoint_exposure = "100"
    with pytest.raises(ValueError, match="Failed to convert budget string to U256"):
        hyperdrive_py.get_max_long(POOL_CONFIG, POOL_INFO, budget, checkpoint_exposure, max_iterations)
...
def test_max_short():
    ...
    max_iterations = 20
    max_short = hyperdrive_py.get_max_short(
        POOL_CONFIG,
        POOL_INFO,
        budget,
        open_share_price,
